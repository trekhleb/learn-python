...
def test_test_variables():
    """Test variables"""

    integer_variable = 100
    float_variable = 5.5
    string_variable = 'Johnson'

    assert integer_variable == 100
    assert float_variable == 5.5
    assert string_variable == 'Johnson'

    variable_with_changed_type = 4
    # x is of type int
    assert variable_with_changed_type == 4
    variable_with_changed_type = 'Sally'
    # x is now of type str
    assert variable_with_changed_type == 'Sally'
    variable_with_changed_type = "5.5"
    # x is now the type str
    assert variable_with_changed_type == "5.5"
    variable_with_changed_type = 5.5
    # x is now the type float
    assert variable_with_changed_type == 5.5
