...
def test_tuples():
    ...
    assert num_tuple[0] == 5
    assert num_tuple[1] == 2
    assert num_tuple[2] == 1

    # You cannot change values in a tuple.
    with pytest.raises(Exception):
        # pylint: disable=unsupported-assignment-operation
        num_tuple[0] = "pineapple"

    # It is also possible to use the tuple() constructor to make a tuple (note the double
    # round-brackets).
    # The len() function returns the length of the tuple.
    num_tuple_via_constructor = tuple((5, 1, 2))

    assert isinstance(num_tuple_via_constructor, tuple)
    assert len(num_tuple_via_constructor) == 3
