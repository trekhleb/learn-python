...
def test_test_list_type():
    ...
    test_cubes = [1, 8, 27, 64, 125]

    assert isinstance(test_cubes, list)

    # Like strings (and all other built-in sequence type), lists can be
    # indexed and sliced:
    assert test_cubes[0] == 1  # indexing returns the item
    assert test_cubes[-1] == 125
    assert test_cubes[-3:] == [27, 64, 125]  # slicing returns a new list

    # All slice operations return a new list containing the requested elements.
    # This means that the following slice returns a new (shallow) copy of
    # the list:
    assert test_cubes[:] == [1, 8, 27, 64, 125]

    # Lists also support operations like concatenation:
    assert test_cubes + [36, 49, 64, 81, 100] == [1, 8, 27, 64, 125, 36, 49, 64, 81, 100]
...
def test_list_methods():
    ...
    numbers = [1, 2, 3, 5, 6, 4, 7, 5]

    # list.append(x)
    # Add an item to the end of the list.
    # Equivalent to a[len(a):] = [x].
    numbers.append(100)
    assert numbers == [1, 2, 3, 5, 6, 4, 7, 5, 100]

    # list.remove(x)
    # Remove the first item from the list whose value is equal to x.
    # It raises a ValueError if there is no such item.
    numbers.remove(6)
    assert numbers == [1, 2, 3, 5, 4, 7, 5, 100]

    with pytest.raises(Exception):
        numbers.remove('not existing element')
    ...
    assert numbers == [-100, 1, 2, 3, 5, 4, 7, 5, 100]
    ...
    assert numbers.index(-100) == 0
    assert numbers.index(1) == 1
    assert numbers.index(5) == 4
    assert numbers.index(5, 5) == 7  # Find next banana starting a position 5

    with pytest.raises(Exception):
        numbers.index('not existing element')

    # list.count(x)
    # Return the number of times x appears in the list.
    assert numbers.count(500) == 0
    assert numbers.count(5) == 2
